...
class LLMJudge:
        ...
        try:
            ...
            if match:
                ...
                for k, v in parsed.items():
                    try:
                        val = float(v)
                        scores[k] = val if val <= 1.0 else val / 5.0
                    except (ValueError, TypeError):
                        scores[k] = 0.5
            else:
                scores = {c: 0.5 for c in rubric}
        except Exception:
            scores = {c: 0.5 for c in rubric}
    ...
    def detect_bias(self, scores_batch: list[dict[str, Any]]) -> dict[str, Any]:
        ...
        for item in scores_batch:
            ...
            all_scores.extend(float(v) for v in item_scores.values())
...
class FailureAnalyzer:
    ...
    def find_root_cause(self, failure: EvalResult) -> str:
        ...
        min_val = min(f, r, c)
        min_scores = [score_name for score_name, score_val in [("f", f), ("r", r), ("c", c)] if score_val == min_val]
    ...
    def generate_improvement_log(self, failures: list, suggestions: list[str]) -> str:
        ...
        for idx, f in enumerate(failures, start=1):
            ...
            sug = suggestions[idx - 1] if idx - 1 < len(suggestions) else "Review full pipeline"
    ...
    def generate_improvement_suggestions(
        ...
        if categories.get("irrelevant", 0) > 0 or any(f.relevance < 0.5 for f in failures):
            suggestions.append("Optimize prompt system message for intent extraction and query clarity")
